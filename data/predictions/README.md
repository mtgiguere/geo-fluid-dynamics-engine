...
- `SEALED_MANIFEST_DRAFT_<date>.csv` — the seal on the playbooks. Ten
  playbooks (five measures x two sides) are generated by
  `targeting.build_playbook` into `sealed/` (git-ignored: a committed file
  is a published file) and only their SHA-256 digests and sizes are
  committed here. After the November scorecard the files are committed and
  `seal.verify_manifest` proves they are byte-for-byte what was sealed.
  Playbook columns: `plan_for`, `bucket` (turnout / persuade / hard for that
  side), `expected_share`, `mean_gap`, `expected_votes`, `votes_per_point`,
  `margin_votes`, `headroom_votes`, `votes_at_risk_midterm`, `priority`.
